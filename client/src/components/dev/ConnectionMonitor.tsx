...
function getAlertSummary(
  alerts: PollingAlert[]
): { severity: PollingAlert['level']; count: number } | null {
  if (alerts.length === 0) {
    return null
  }
...
function getEventAccent(
  type: PollingMetricsSnapshot['events'][number]['type']
): string {
  switch (type) {
    case 'error':
      return 'text-red-600'
...
export function ConnectionMonitor({
  ...
  return (
    <div
      className={`bg-gray-50 border border-gray-200 rounded-lg shadow-sm ${className}`}
    >
      <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200 rounded-t-lg">
        ...
        >
          <span
            className="text-lg bg-gray-500 text-white px-1.5 py-0.5 rounded"
            style={{ fontSize: '16px' }}
          >
            🔧
          </span>
          ...
          {metrics && (
            <span className="flex items-center gap-3 text-xs font-mono text-blue-500">
              <span className={errorRateClass}>
                {formatPercentage(errorRate)} err
              </span>
              <span
              ...
              <div className="bg-white rounded border p-2">
                <div className="flex flex-wrap items-center gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-semibold ${complianceStyle.className}`}
                    >
                  ...
                  {alertSummary && (
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-semibold ${getAlertClass(
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {sortedEndpoints.map((endpoint) => (
                          <tr key={endpoint.key} className="text-gray-700">
                            <td className="px-2 py-1 font-semibold text-gray-800 text-xs">
                              {endpoint.label}
                            </td>
                            ...
                            <td
                              className={`px-2 py-1 font-semibold text-xs ${getErrorRateColor(
                                endpoint.errorRate
                              )}`}
                            >
                            ...
                            <td
                              className={`px-2 py-1 font-semibold text-xs ${getLatencyColor(
                            ...
                            <td className="px-2 py-1 text-xs">
                              {formatTimestamp(endpoint.lastSuccessAt)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              {metrics.events.length > 0 && (
                <div className="bg-white rounded border">
                  <div className="px-2 py-1.5 bg-gray-50 border-b text-xs font-medium text-gray-700 uppercase tracking-wide">
                    Recent Activity ({metrics.events.length} event
                    {metrics.events.length === 1 ? '' : 's'})
                  </div>
                  <div className="max-h-32 overflow-y-auto divide-y divide-gray-100">
                    {metrics.events
                      ...
                      .map((event, index) => (
                        ...
                        >
                          <span className="font-mono text-gray-400 w-16 text-[10px]">
                            {formatTimestamp(event.timestamp)}
                          </span>
                          <span
                            className={`uppercase tracking-wide font-semibold text-[10px] ${getEventAccent(
                              event.type
                            )}`}
                          >
                            {event.type}
                          </span>
                          <span className="text-gray-700 flex-1 text-[11px]">
                            {event.message}
                          </span>
                          {event.details && (
